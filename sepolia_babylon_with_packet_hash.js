...
async function main() {
  ...
  for (const w of wallets) {
    ...
    try {
      ...
      console.log(`🚀 Sending from ${wallet.address} (${name})`);
      ...
      await tx.wait();
	  console.log(`${timelog()} | ✅ ${name} | Transaction Confirmed: ${explorer.tx(tx.hash)}`);
	  const txHash = tx.hash.startsWith("0x") ? tx.hash : `0x${tx.hash}`;
	  const packetHash = await pollPacketHash(txHash);
	  console.log(`${timelog()} | ✅ ${name} | Packet Submitted: ${unionExplorer.tx(packetHash)}`);
	  console.log('');
    } catch (err) {
