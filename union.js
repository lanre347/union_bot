...
async function selectWallets(wallets) {
  ...
  for (const w of selected) {
    try {
      ...
      console.error(`[${timelog()}] Wallet "${w.name}" has invalid private key. Skipping.`);
    }
  }

  if (validWallets.length === 0) {
    console.error(`[${timelog()}] No valid wallets found. Exiting.`);
    process.exit(1);
  }
...
async function runTransactionParallel() {
  ...
  for (const tx of transactionTypes) {
    console.log(`[${timelog()}] Executing transaction: ${tx.label}`);
    try {
      await tx.method();
    } catch (error) {
      console.error(`[${timelog()}] Error in ${tx.label}: ${error.message}`);
    }
  }
}
